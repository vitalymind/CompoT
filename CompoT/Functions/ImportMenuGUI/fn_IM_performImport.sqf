...
private ["_dialog","_importList","_collisionOption","_importType","_importData"];
...
if (count CT_var_IM_importOptions == 0) exitWith {["Import data is not loaded"] call CT_fnc_errorMsg};
{
	...
	if ((_importType select 0) == 1) then {
	...
	if ((_importType select 0) == 2) then {
		if ((_importType select 1) == 0) then {["object",_collisionOption,_importData] call CT_fnc_IM_importElement};
		if ((_importType select 1) == 1) then {["composition",_collisionOption,_importData] call CT_fnc_IM_importElement};
		if ((_importType select 1) == 2) then {["collection",_collisionOption,_importData] call CT_fnc_IM_importElement};
		if ((_importType select 1) == 3) then {["prefab",_collisionOption,_importData] call CT_fnc_IM_importElement};
	};
	if ((_importType select 0) == 3) then {
		if ((_importType select 1) == 0) then {[_importData] call CT_fnc_IM_importProject};
	};
