...
"_dateEdit","_typeEdit","_authorEdit","_importList","_index","_type","_options","_authorText","_destinationEdit"];
...
_destinationEdit ctrlSetText ((CT_var_IM_importOptions select _index) select 6);
_type = ((CT_var_IM_importOptions select _index) select 4);
_options = ((CT_var_IM_importOptions select _index) select 5);
if ((str _type) in ["[0]","[1,0]","[1,1]","[1,2]","[1,3]","[2,0]","[2,1]","[2,2]","[2,3]","[3,0]","[3,1]","[3,2]","[3,3]"]) then {
	{ctrlShow [_x, true]}forEach [501,502,503,504,203,204,205];
	{ctrlShow [_x, false]}forEach [305,505];
};
if ((str _type) in ["[2,0]","[2,1]","[2,2]","[2,3]"]) then {
	{ctrlShow [_x, true]}forEach [305,505];
};
