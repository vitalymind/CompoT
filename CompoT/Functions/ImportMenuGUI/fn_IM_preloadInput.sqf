...
CT_var_IM_importOptions = [];
{
	_name = ""; _name = switch true do {
		...
		case ((_x select 4) isEqualTo [3,0]): {"Category, object"};
		case ((_x select 4) isEqualTo [3,1]): {"Category, composition"};
		case ((_x select 4) isEqualTo [3,2]): {"Category, collection"};
		case ((_x select 4) isEqualTo [3,3]): {"Category, prefab"};
	};
	_index = _importList lbAdd _name;
	_importList lbSetData [_index, str(_x select 5)];
	CT_var_IM_importOptions pushBack [(_x select 0),(_x select 1),(_x select 2),(_x select 3),(_x select 4),2,"import"];
} forEach _import;
