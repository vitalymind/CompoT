/* 
   File: fn_PrL_move.sqf 
   Function: CT_fnc_PrL_move 
...
_direction = _this;
_curList = CT_var_PrL_selection;
if (_curList == "prefab") then {
...
if (_curList == "category") then {
	_targetPos = _targetPos max 0 min (count CT_var_prefabs - 1);
} else {
...
if (_curList == "category") then {
	[CT_var_prefabs, _currentPos, _targetPos] call _shift;
} else {
